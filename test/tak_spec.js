...
const testProtoJSON = {
  cotEvent: {
    ...
    detail: {
      xmlDetail:
        '<contact endpoint="*:-1:stcp" callsign="VPN User" sipAddress="192.168.0.54"/><uid Droid="VPN User"/>',
    ...
    helper.load(makeTAKNode, flow, () => {
      ...
      o0.on("input", (msg) => {
        try {
          ...
          msg.payload.event.point._attributes.lat.should.equal(
            testJSON.event.point._attributes.lat
          );
          msg.payload.event.point._attributes.lon.should.equal(
            testJSON.event.point._attributes.lon
          );
    ...
    helper.load(makeTAKNode, flow, () => {
      ...
      o1.on("input", (msg) => {
        try {
          ...
          controlVal = Buffer.from([
            0xbf, 0x01, 0xbf, 0x12, 0xd5, 0x02, 0x0a, 0x0b, 0x61, 0x2d, 0x66,
          ]);

          // Check header:
          Buffer.compare(
            payload.slice(0, 3),
            controlVal.slice(0, 3)
          ).should.equal(0);
